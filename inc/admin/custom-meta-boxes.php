...
function juizl_custom_meta_boxes( $post ) {
	...
	add_meta_box(
		'juizl-hreflang',
		esc_html__( 'Custom HREFLANG', 'juiz-lang' ),
		'juizl_hreflang_mb',
		null,
		'normal',
		'high',
...
function juizl_hreflang_mb( $post ) {
	...
	$output  = '<p class="juizl-mb-description">' . sprintf( esc_html__( 'You can provide alternate URL to Google if your content is translated somewhere in the Web. Fill code with 2-letters (%s for English) code or 4-letters (%s for English in United State) code if you specify the region.', 'juiz-lang' ), '<code>en</code>', '<code>en-US</code>' ) . '</p>';
	...
	$delbtn = '<button type="button" class="juizl-remove-link-btn juizl-button juizl-button-mini" aria-controls="juizl-other-links"><span class="screen-reader-text">' . esc_html__( 'Remove this link', 'juiz-lang' ) . '</span><i class="dashicons dashicons-no-alt" aria-hidden="true"></i></button>';
	...
	$output .= '<p class="juizl-mb-line juizl-inlined-inputs' . esc_attr( $c === $i || $c === 0 ? ' juizl-to-duplicate' : '' ) . '">
			<span class="juizl-small-col">
				<label for="juizl-hreflang-code-' . $i . '">' . esc_html__( 'Language Code', 'juiz-lang' ) . $doclink . '</label><br>
				<input type="text" name="juizl-hreflang[code][]" id="juizl-hreflang-code-' . intval( $i ) . '" value="' . ( $c > 0 && isset( $juizl_hreflangs['code'][0] ) ? esc_attr( $juizl_hreflangs['code'][0] ) : '' ) . '" />
			</span>
			<span class="juizl-big-col">
				<label for="juizl-hreflang-href-' . intval( $i ) . '">' . esc_html__( 'URL', 'juiz-lang' ) . '</label><br>
	...
	if ( ! empty( $juizl_hreflangs ) && is_array( $juizl_hreflangs ) && isset( $juizl_hreflangs['code'] ) && $c > 1 ) {
		...
		foreach ( $juizl_hreflangs['code'] as $lang ) {
			...
			$output .= '<p class="juizl-mb-line juizl-inlined-inputs' . ( $c === $i ? ' juizl-to-duplicate' : '' ) . '">
					<span class="juizl-small-col">
						<label for="juizl-hreflang-code-' . $i . '">' . esc_html__( 'Language Code', 'juiz-lang' ) . $doclink . '</label><br>
						<input type="text" name="juizl-hreflang[code][]" id="juizl-hreflang-code-' . intval( $i ) . '" value="' . esc_attr( $lang ) . '" />
					</span>
					<span class="juizl-big-col">
						<label for="juizl-hreflang-href-' . $i . '">' . esc_html__( 'URL', 'juiz-lang' ) . '</label><br>
