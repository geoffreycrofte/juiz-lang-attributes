	...
	/*var juizlHreflangAttribute = function( props ) {
		return wp.element.createElement(
			wp.editor.RichTextToolbarButton, {
				...
				title: props.isActive ? 'Remove HREFLANG Attribute' : 'Add HREFLANG Attribute',
				onClick: function() {
					...
					// TODO: selected or not selected, should toggling.
					//if ( 0 === selText.length && ! props.isActive ) {
					//	return;
					//}

					if ( props.isActive ) {
						const activeFormat = wp.richText.getActiveFormat(props.value, 'juiz-lang/hreflang-attribute');
						
						hreflangVal = activeFormat.attributes.hreflang;
					}

					hreflangVal = hreflangVal ? hreflangVal : prompt('What HREFLANG value?');
					...
					if ( hreflangVal ) {
						props.onChange( 
							wp.richText.toggleFormat(
								props.value,
								{
									type: 'juiz-lang/hreflang-attribute',
									attributes: {
